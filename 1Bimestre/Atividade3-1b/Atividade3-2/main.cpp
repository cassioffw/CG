...
void salvarArquivo() {
    //criando o arquivo
    std::ofstream arquivo("C:/Users/Cassio/Desktop/arquivo_teste.txt");
    //verifica se o arquivo foi criado e está aberto corretamente
    if (!arquivo.is_open()) {
        std::cerr << "Erro ao abrir o arquivo!" << std::endl; //erro
        return;
    }

    for (auto ponto = pontos.begin(); ponto != pontos.end(); ponto++) {
        //adicionando as coordenadas no arquivo
        arquivo << (double)ponto->x << " " << (double)ponto->y << std::endl;
    }
...
void abrirArquivo() {
    std::ifstream inFile("C:/Users/Cassio/Desktop/arquivo_teste.txt");
    ...
    while (!inFile.eof()) {
        inFile >> x >> y; // salva as coordenadas do arquivo nas variaveis
        pontos.push_back({ x, y }); //adiciona os pontos na lista
    }
...
void keyboard(unsigned char key, int x, int y) {
    if (key == 's' || key == 'S') { //se a tecla precionada for a 'S'
        salvarArquivo(); //chama a função de salvar arquivo
    }
    else if (key == 'o' || key == 'O') { //se a tecla for 'O'
        abrirArquivo();// chama a função de abrir arquivo
    }
    else {
        return;
    }
